# Codecs
import codecs
codec = codecs.getreader("utf-8")
#
# URLS
URL = "https://api.lootbox.eu/"
#
# Commands
HELP = ("gethelp", "get_help", "get_halp", "help", "h")
PATCH_NOTES = ("getpatchnotes", "get_patchnotes", "patchnotes", "patch_notes", "pn", "patch")
ACHIEVEMENTS = ("get_achievements", "getachievement", "getachievements", "achievements", "achieve", "a")
PLATFORMS = ("get_platforms", "getplatform", "getplatforms","platforms","platform", "pl")
PROFILES = ("get_profile", "getprofile", "getprofiles", "profile", "profiles", "pr")
ALL_HEROES_STATS = ("get_all_heroes_stats", "get_heroes_stats", "get_heroe_stats", "getheroesstats", "getallheroestats", "getallheroetats", "getstats", "ghs")
GET_MEDALS = ("getmedals", "get_medals", "gm")
GET_KILLS = ("getkills", "get_kills", "gk")
GET_TIME = ("gettime", "get_time", "gt")
GET_GAMES = ("getgames", "get_games", "gg")
GET_OBJECTIVES = ("getobjectives", "get_objectives", "go")
#
# Constants
LATEST = ("latest", "lat", "LATEST", "LAT")